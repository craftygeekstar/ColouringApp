...
public class BrushStroke : MonoBehaviour
{
    ...
    private void AddRibbonPointIfNeeded() 
    {
        ...
        if (Vector3.Distance(_ribbonEndPosition, _previousRibbonPointPosition) >= 0.01f || Quaternion.Angle(_ribbonEndRotation, _previousRibbonPointRotation) >= 10.0f) 
        {
    ...
    private void AnimateLastRibbonPointTowardsBrushTipPosition() 
    {
        ...
        if (Vector3.Distance(_ribbonEndPosition, brushTipPosition) <= 0.0001f && Quaternion.Angle(_ribbonEndRotation, brushTipRotation) <= 0.01f) 
        {
            return;
        }
